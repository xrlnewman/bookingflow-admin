...
function mobileView() {
  return `<section class="mobile-panel"><div class="mobile-panel__hero"><span>BOOKINGFLOW MOBILE</span><h2>我的预约与到店服务</h2><p>移动端与预约 API 共享服务目录、时段容量、改约、退款和评价记录。</p><button class="primary" data-action="create-appointment">＋ 创建演示预约</button></div><div class="mobile-list"><h3>今日预约</h3>${appointments.slice(0, 4).map((appointment) => `<article class="mobile-card"><div><small>${timeLabel(appointment.scheduledAt)} · ${appointment.department}</small><strong>${appointment.patient}</strong><span>${appointment.doctor} · ${appointment.status}</span></div><b class="status ${statusColors[appointment.status] || 'indigo'}">${appointment.status}</b>${appointmentAction(appointment)}</article>`).join('')}</div><div class="mobile-list"><h3>服务回访</h3>${followupTasks.slice(0, 3).map((item) => `<article class="mobile-card"><div><small>${item.dueAt}</small><strong>${item.summary}</strong><span>${item.patient} · ${item.status}</span></div>${item.status === '已完成' ? '<b class="status green">已完成</b>' : `<button class="text-action" data-action="complete-followup" data-followup-id="${item.id}">完成回访</button>`}</article>`).join('')}</div></section>`
}
