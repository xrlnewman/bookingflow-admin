...
BookingFlow 是预约排班与上门服务运营后台，覆盖服务目录、时段容量、预约订单、改约/取消、退款标记、评价、负责人排班和服务跟进。门店人员管理服务与容量，负责人处理订单和履约状态。
...
后台的“服务目录”“预约订单”“预约队列”和“回访任务”按钮会优先调用真实 API；API 暂不可用时保留内置演示数据并提示当前数据来源。侧栏“移动端体验”提供顾客与前台的窄屏视图，支持选服务时段、创建订单、确认、签到、服务完成、改约、取消、退款和评价，便于用手机浏览器联调。
...
BookingFlow 覆盖目录、锁时段、创建、确认、签到、履约、完成、改约、取消、退款标记、评价和回访的预约服务操作；所有演示数据均为虚构，不接入真实财务、人事或客户隐私。
